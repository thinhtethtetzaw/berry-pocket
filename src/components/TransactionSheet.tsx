...
export function TransactionSheet({ visible, onClose, onSave, editing, onUpdate, onDelete, defaultMain = 'living' }: Props) {
  ...
  useEffect(() => {
    if (!visible) return;
    if (editing) {
      setMain(editing.main); setCat(editing.cat);
      setDesc(editing.desc); setAmount(String(editing.amount)); setDate(editing.date);
    } else {
      setDate(todayISO()); setDesc(''); setAmount('');
      setMain(defaultMain);
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
              ...
              <View style={[S.amountRow, { backgroundColor: accentBg }]}>
                <Txt variant="headingMd" color={accentColor}>฿</Txt>
                <TextInput
                  value={amount}
                  ...
                  {visibleCategories.map(m => {
                    ...
                    return (
                      <Pressable
                        ...
                        style={[S.pill, { backgroundColor: active ? accent : '#FFFFFF', borderColor: active ? accent : theme.border }]}
                      >
                        <Icon name={m.icon} size={13} color={active ? '#FFFFFF' : theme.steel} strokeWidth={2} />
                        <Txt variant="bodySmMed" color={active ? '#FFFFFF' : theme.ink}>{m.label}</Txt>
                      </Pressable>
                    );
                  })}
                </View>
                    ...
                    {subs.map(s => {
                      ...
                      return (
                        <Pressable
                          key={s.id}
                          onPress={() => setCat(s.id)}
                          style={[S.pill, S.pillSm, { backgroundColor: active ? accent + '15' : '#FFFFFF', borderColor: active ? accent : theme.border }]}
                        >
                          <Icon name={s.icon} size={12} color={active ? accent : theme.steel} strokeWidth={2} />
                          <Txt variant="caption" color={active ? accent : theme.steel}>{s.label}</Txt>
                        </Pressable>
                      );
                    })}
                  </View>
              ...
              <TextInput
                ...
                placeholder={isIncome ? 'e.g. Monthly salary' : 'e.g. Lunch at MBK'}
              ...
              <View style={S.actions}>
                ...
                <Pressable onPress={submit} style={[S.btn, S.btnPrimary, { backgroundColor: accentColor }]}>
                  <Txt variant="buttonMd" color="#FFFFFF">{isEdit ? 'Save Changes' : 'Add'}</Txt>
                </Pressable>
              </View>
            </ScrollView>
...
function FieldLabel({ label }: { label: string }) {
  const { theme } = useTheme();
  return (
    <Txt variant="microBold" color={theme.steel} style={FL.label}>{label.toUpperCase()}</Txt>
  );
}
