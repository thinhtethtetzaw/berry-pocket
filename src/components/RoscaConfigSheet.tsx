            ...
            >
              ...
              <View
                style={[
                  S.summary,
                  {
                    backgroundColor: ROSCA_ACCENT + "14",
                    borderColor: ROSCA_ACCENT + "40",
                  },
                ]}
              >
                <View style={S.sumRow}>
                  <Txt variant="caption" color={theme.steel}>
                    I'll receive
                  </Txt>
                  <Txt variant="bodyMdBold" color={ROSCA_ACCENT}>
                    {fmt(total)}
                  </Txt>
                </View>
                <View style={S.sumRow}>
                  ...
                  <Txt variant="bodyMd" color={theme.ink}>
                    {MONTHS_SHORT[payoutMonth]} {payoutYear}
                  </Txt>
                </View>
              </View>
              ...
              <NumberField
                ...
                onCommit={(v) => update("monthlyPayment", v)}
                min={0}
                theme={theme}
              />
              ...
              <View style={S.row}>
                <View style={{ flex: 1 }}>
                  <NumberField
                    ...
                    onCommit={(v) => {
                      update("groupSize", v);
                      if (draft.myPosition > v) update("myPosition", v);
                    }}
                    min={1}
                    theme={theme}
                  />
                ...
                <View style={{ flex: 1 }}>
                  <NumberField
                    label={`Position (1–${draft.groupSize})`}
                    value={draft.myPosition}
                    onCommit={(v) =>
                      update(
                        "myPosition",
                        Math.max(1, Math.min(draft.groupSize, v)),
                      )
                    }
                    min={1}
                    max={draft.groupSize}
                    ...
                    {MONTHS_SHORT.map((m, i) => {
                      ...
                      return (
                        <Pressable
                          ...
                          style={[
                            S.monthCell,
                            active && { backgroundColor: theme.ink },
                          ]}
                        >
                          <Txt
                            variant="microBold"
...
function NumberField({
  ...
  return (
    <View style={[S.field, noLabel && { marginTop: 0 }]}>
      {!noLabel && (
        <Txt variant="microBold" color={theme.steel} style={S.fieldLabel}>
          {label.toUpperCase()}
        </Txt>
      )}
      <NumberInput
...
const S = StyleSheet.create({
  ...
  row: { flexDirection: "row", gap: space.xs, alignItems: "flex-end" },
